...
Wine patches live under `wine/` and are applied to CrossOver's Wine 11.0 source by `scripts/build-wine.sh apply`, in this order: `wine/dwproton/em-backports/*` → `wine/dwproton/misc/*` → `wine/macos/*`.

### `dwproton/`

Endfield patches from dwproton commit `b816be489` in [dawn-winery/dwproton-mirror](https://github.com/dawn-winery/dwproton-mirror). Fetch with `scripts/fetch-dwproton-patches.sh`; set `DWPROTON_SHA` to use another commit.
...
[Wine](https://www.winehq.org/) patches are licensed LGPL-2.1-or-later; `moltenvk/` patches, Apache-2.0 like MoltenVK and SPIRV-Cross. `wine/dwproton/*` retain upstream authorship (Etaash Mathamsetty, Ziia Shi / mkrsym1, NelloKudo and other dwproton contributors).
